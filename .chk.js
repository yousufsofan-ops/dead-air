...
let S=loadSave();if(S.set.smooth===undefined)S.set.smooth=0.35;if(S.set.tp===undefined)S.set.tp=false;if(!S.hints)S.hints={};if(!S.cosOwned)S.cosOwned={};if(S.prestige===undefined)S.prestige=0;if(S.ending===undefined)S.ending=false;if(S.set.voiceAuto===undefined)S.set.voiceAuto=true;if(S.set.skipHome===undefined)S.set.skipHome=false;if(S.set.hitstop===undefined)S.set.hitstop=true;if(!S.set.binds||typeof S.set.binds!=='object')S.set.binds={};if(S.set.sprintToggle===undefined)S.set.sprintToggle=false;if(S.set.padSens===undefined)S.set.padSens=1;if(S.lastDied===undefined)S.lastDied=false;if(S.lastSite===undefined)S.lastSite='';if(!S.zeroBest)S.zeroBest=0;if(!S.stats.seenKinds)S.stats.seenKinds={};if(S.set.touch===undefined)S.set.touch=!!(navigator.maxTouchPoints>0&&window.matchMedia&&matchMedia('(pointer:coarse)').matches);if(S.set.gfx===2&&!S.set.gfxMig){S.set.gfx=3;S.set.gfxMig=true;}
...
const CREATURES={
 ...
 stack:{n:'The Stack',ic:'📦',major:false,threat:1,d:'Looks like loot. Is not loot. Hold your light on a pile for a second: real loot does not twitch.'},
 ...
 collector:{n:'The Collector',ic:'🎒',major:false,threat:1,d:'Steals loose loot and drags it to a nest. It jingles while it carries. Throw anything at it or light a flare and it drops the loot and runs.'},
 sleeper:{n:'The Sleeper',ic:'😴',major:true,threat:4,d:'Sleeps on a pile of treasure on the deepest floor. Noise and light wake it; crouching near it barely does. When it stirs, back away crouched with the light off and it settles.'},
...
const Aud={ctx:null,master:null,sfx:null,mus:null,voiceBus:null,noiseBuf:null,conv:null,revSend:null,revSendV:null,music:{lvl:0,cur:0,t:0,target:0},ambNodes:[],ambKind:null,ambT:2,hb:{t:0,rate:0},
 init(){if(this.ctx){if(this.ctx.state==='suspended')this.ctx.resume();return this.ctx;}
  try{const c=new (window.AudioContext||window.webkitAudioContext)();this.ctx=c;
   this.master=c.createGain();this.master.gain.value=this.vcurve(S.set.vol);const comp=c.createDynamicsCompressor();comp.threshold.value=-14;comp.ratio.value=4;comp.attack.value=0.004;comp.release.value=0.2;this.master.connect(comp);comp.connect(c.destination);
   this.sfx=c.createGain();this.sfx.connect(this.master);
   this.mus=c.createGain();this.mus.gain.value=this.vcurve(S.set.music);this.mus.connect(this.master);
  ...
  return this.ctx;},
 setVol(){if(!this.ctx)return;this.master.gain.value=this.vcurve(S.set.vol);{const g=this.mus.gain;g.cancelScheduledValues(this.ctx.currentTime);g.value=this.vcurve(S.set.music);}this.voiceBus.gain.value=S.set.voiceVol;},
...
function updateCreaturesHost(dt){if(G.mission&&G.mission.zero)sigTick(dt);const calm=G.t<G.director.calmUntil;const agg=(G.mission.diff.creat||1)*(1+G.director.aggro*0.5)*(G.mission.weekly==='angry'?1.5:1)*(G.stage>=3?1.25:1);const hearMul=(calm?0.5:1)*agg;
  ...
  for(const c of G.creatures.slice()){c.timer-=dt;c.cool-=dt;const cfg=CR_CFG[c.kind];const spd=cfg.speed*(calm?0.85:1),hunt=cfg.hunt*(calm?0.85:1)*(G.stage>=6?1.15:1);
    ...
    case 'collector':{
      ...
      else if(c.state==='carry'){c.anim='run';if(!carried){c.state='wander';break;}if(Math.random()<dt*0.8){if(MP.on)bcast({t:'ev',k:'sfx',kind:'collector',p:[c.pos.x,c.pos.y+1,c.pos.z]});Aud.creature('collector',c.pos,0.45);}if(moveTo(c,c.nest.x,c.nest.z,c.nest.k,hunt*0.9,dt)||c.timer<=0){carried.carrier=null;carried.sleep=false;carried.p.set(c.nest.x+(Math.random()-0.5)*1.5,groundY(G.map,c.k,c.nest.x,c.nest.z)+0.3,c.nest.z+(Math.random()-0.5)*1.5);carried.k=c.k;carried.v.set(0,1,0);c.item=null;c.state='wander';c.cool=4;}}
      ...
      break;}
    case 'sleeper':{
      if(c.state==='sleep'){c.anim='idle';for(const p of alivePlayers()){if(p.k!==c.k)continue;const d=p.pos.distanceTo(c.pos);if(d<6)c.awake+=dt*0.12*(6-d)/6*(p.crouch?0.35:1)*(p.flash?1.3:1);if(p.flash&&d<10&&losClear(G.map,c.k,c.pos.x,c.pos.z,p.pos.x,p.pos.z))c.awake+=dt*0.07;}
        ...
        c.awake=Math.max(0,c.awake-dt*0.012);if(c.awake>0.6&&!c.stirWarned){c.stirWarned=true;const np=nearestPlayer(c);if(np)hostEvent(np.p.id,{k:'toast',s:'😴 It is stirring. Back away crouched, light off, and it settles.'});}
...
function updateLights(dt){updateFloorCulling();{const rid=roomAt(G.map,P.k,P.pos.x,P.pos.z);const r=G.map.layers[P.k]&&G.map.layers[P.k].rooms.find(q=>q.id===rid);const was=P.inSafe;P.inSafe=!!(r&&r.safe);if(P.inSafe&&!was&&G.state==='play')hintOnce('safe','This is a breather room: the lights here never fail, nothing spawns inside, and you heal slowly (up to 60). Creatures can still follow you in.');}if(frame%2===0)updateGlowPool();const pal=G.map.loc.pal;const cands=[];for(const l of G.lights){if(l.k!==P.k)continue;const d=dist2(l.x,l.z,P.pos.x,P.pos.z);if(d<24)cands.push([d,l]);}cands.sort((a,b)=>a[0]-b[0]);
  ...
  const n=G.lightPool.length;for(let i=0;i<n;i++){const pl=G.lightPool[i];const c=cands[i+off];if(!c){pl.intensity=0;continue;}const l=c[1];pl.position.set(l.x,l.y-0.2,l.z);
    if(l.on){let it=1.25;if(l.flicker&&Math.sin(G.t*23+l.phase)*Math.sin(G.t*7.3+l.phase)>0.6)it*=0.15;if(G.fx.lightsOut>0&&!l.safe)it=0;pl.color.setHex(l.vault?0xff9060:pal.light);pl.intensity=it;pl.distance=15;if(l.tube){l.tube.emissive.setHex(l.vault?0xff9060:0xfff2d8);l.tube.emissiveIntensity=it>0.5?1:0.1;}}
...
function runEvent(id,quiet,chosen){const data={};const pls=alivePlayers();const tp=chosen||(pls.length?pick(pls):null);
  switch(id){
    ...
    case 'creature':{if(!tp)return;const kind=pick(['hound','drifter','stack','drifter']);const rp=spawnCell(ri(Math.random,0,G.map.L-1));spawnCreature(kind,rp.x,rp.z,rp.k);if(kind==='drifter'){spawnCreature('drifter',rp.x+1,rp.z+1,rp.k);}return;}
    ...
    case 'fakeout':{if(!tp)return;const a=Math.random()*TAU,d=2.5+Math.random()*2;data.p=[tp.pos.x+Math.sin(a)*d,layerY(tp.k)+1,tp.pos.z+Math.cos(a)*d];data.pid=tp.id;
      /* the real one: six to ten seconds after everyone relaxes */const D=G.director;D.pending.push({t:G.t+6+Math.random()*4,fn:()=>{const pl=alivePlayers().find(p=>p.id===tp.id);if(!pl||pl.inVan)return;const eco=(G.mission.eco&&G.mission.eco.length)?G.mission.eco.filter(k=>['hound','echo','stack','crawler','collector','sleeper'].indexOf(k)>=0):[];const kind=eco.length?pick(eco):'hound';const b=Math.random()*TAU;const rp=spawnCell(pl.k);spawnCreature(kind,rp.x,rp.z,pl.k,{state:kind==='hound'?'chase':'hunt'});const c=G.creatures[G.creatures.length-1];if(c){c.target=pl.id;c.lastSeen=G.t;c.lastHeard={x:pl.pos.x,z:pl.pos.z,k:pl.k,t:G.t,loud:1};}runEvent('lightsout');}});break;}
  }
...
function setStage(st){G.stage=st;const names=['','EXPLORATION','ANOMALIES','ACTIVITY','LIGHTS FAILING','MAJOR ENTITY','COLLAPSE','EMERGENCY EXTRACTION'];$('stageN').textContent='STAGE '+st;$('stageT').textContent=names[st];$('stage').className=st>=6?'crit':st>=4?'warn':'';
  ...
  if(st===5&&(mpHost()||!MP.on)){const hasMajor=G.creatures.some(c=>CREATURES[c.kind].major);const rp=spawnCell(hasMajor?ri(Math.random,0,G.map.L-1):0);spawnCreature(hasMajor?'hound':'warden',rp.x,rp.z,rp.k);}
