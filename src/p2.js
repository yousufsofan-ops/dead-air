...
let S=loadSave();if(S.set.smooth===undefined)S.set.smooth=0.35;if(S.set.tp===undefined)S.set.tp=false;if(!S.hints)S.hints={};if(!S.cosOwned)S.cosOwned={};if(S.prestige===undefined)S.prestige=0;if(S.ending===undefined)S.ending=false;if(S.set.voiceAuto===undefined)S.set.voiceAuto=true;if(S.set.skipHome===undefined)S.set.skipHome=false;if(S.set.hitstop===undefined)S.set.hitstop=true;if(!S.zeroBest)S.zeroBest=0;if(!S.stats.seenKinds)S.stats.seenKinds={};if(S.set.touch===undefined)S.set.touch=!!(navigator.maxTouchPoints>0&&window.matchMedia&&matchMedia('(pointer:coarse)').matches);if(S.set.gfx===2&&!S.set.gfxMig){S.set.gfx=3;S.set.gfxMig=true;}
