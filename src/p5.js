...
function creatureDoors(c,hx,hz,speed,dt){
  ...
  return false;}
function setDoorHost(d,o,slam){d.target=o;const p=new THREE.Vector3(d.x,layerY(d.k)+1,d.z);Aud.door(p,slam);if(MP.on)bcast({t:'ev',k:'door',id:d.id,o,slam:!!slam});}
...
function updateCreaturesHost(dt){if(G.mission&&G.mission.zero)sigTick(dt);const calm=G.t<G.director.calmUntil;const agg=(G.mission.diff.creat||1)*(1+G.director.aggro*0.5)*(G.mission.weekly==='angry'?1.5:1)*(G.stage>=3?1.25:1);const hearMul=(calm?0.5:1)*agg;
  ...
  for(const c of G.creatures.slice()){c.timer-=dt;c.cool-=dt;const cfg=CR_CFG[c.kind];const spd=cfg.speed*(calm?0.85:1),hunt=cfg.hunt*(calm?0.85:1)*(G.stage>=6?1.15:1);
    ...
    case 'echo':{
      ...
      else if(c.state==='stalk'){const h=c.lastHeard;if(!h||moveTo(c,h.x,h.z,h.k,spd*1.9,dt)){c.state='wander';c.wp=null;}c.anim='walk';}
      ...
      if(c.state!=='attack'&&c.state!=='retreat'){for(const p of alivePlayers()){if(p.loud>0.3&&canSee(c,p)){c.state='hunt';c.target=p.id;c.lastSeen=G.t;G.mission.echoHunted=true;break;}}}
    ...
    case 'hound':{
      ...
      if(c.state!=='chase'&&c.state!=='recoil'&&c.state!=='circle'&&c.state!=='cower'&&c.state!=='fetch'){for(const p of alivePlayers()){if(canSee(c,p)){c.state='chase';c.target=p.id;c.lastSeen=G.t;if(MP.on)bcast({t:'ev',k:'sfx',kind:'hound_bark',p:[c.pos.x,c.pos.y,c.pos.z]});Aud.creature('hound_bark',c.pos,1.4);break;}}}
...
/* ================= LIGHTS ================= */
function updateLights(dt){const pal=G.map.loc.pal;const cands=[];for(const l of G.lights){if(l.k!==P.k)continue;const d=dist2(l.x,l.z,P.pos.x,P.pos.z);if(d<24)cands.push([d,l]);}cands.sort((a,b)=>a[0]-b[0]);
...
function initDirector(){G.director={aggro:0,calmUntil:0,nextEvent:25+Math.random()*20,seen:{},tick:0,distantT:20};}
function directorTick(dt){const D=G.director;D.aggro=Math.max(0,D.aggro-dt*0.02);D.tick+=dt;
  D.nextEvent-=dt*(1+G.stage*0.12)*(G.mission.sandbox?0.4:1);
  if(D.nextEvent<=0){D.nextEvent=45+Math.random()*45;if(G.t>=D.calmUntil||Math.random()<0.3){const pool=EVENT_DEFS.filter(e=>e.minStage<=G.stage&&!(G.mission.sandbox&&e.id==='creature'));const ev=wpick(pool,Math.random,e=>e.w*Math.pow(0.35,D.seen[e.id]||0)*(S.seenEvents[e.id]?0.7:1.35));D.seen[ev.id]=(D.seen[ev.id]||0)+1;S.seenEvents[ev.id]=(S.seenEvents[ev.id]||0)+1;runEvent(ev.id);}}
  D.distantT-=dt;if(D.distantT<=0){D.distantT=14+Math.random()*25;runEvent('distant',true);}
}
function runEvent(id,quiet){const data={};const pls=alivePlayers();const tp=pls.length?pick(pls):null;
  switch(id){
...
function updateLightsFx(dt){updateVanLock(dt);updateDissolve(dt);SHU.time.value=G.t;
  ...
  if(G.edgePass){const u=G.edgePass.uniforms;u.time.value=G.t;u.uRain.value=(P.k===0&&cellAt(G.map,0,P.pos.x,P.pos.z)===T_YARD)?1:0;u.uAberr.value=G.fx.glitch>0?0.012*Math.min(1,G.fx.glitch):(G.fx.shake>0.4?0.003:0);u.uHurt.value=Math.min(1,(P.hp<35&&!P.dead?(1-P.hp/35)*0.7:0)+Math.max(0,P.hurtT)*0.6);u.uBlind.value=P.blind>0?Math.min(1,P.blind/2):0;}
